...
const Header: React.FC<any> = ({
  ...
  const headerClasses = cn(
    ...
    variant === "floating"
      ? cn(
          "right-0 mx-auto max-w-5xl rounded-full border border-black/10 bg-white/80 backdrop-blur-md px-6 py-3 shadow-2xl w-[95%]",
          !isScrolled && hasAnnouncement && !isPreview ? "mt-4" : "mt-4"
        )
      : cn("border-b border-transparent py-4"),
    variant !== "floating" && isSticky && isScrolled
      ? "bg-white/90 backdrop-blur-md border-black/10 py-3 shadow-lg top-0"
      : "",
  ...
  const Logo = () => (
    ...
    >
      {data.logoImage ? (
        ...
        <InlineEdit
          tagName="span"
          className="text-xl md:text-2xl font-bold tracking-tighter text-transparent bg-clip-text bg-gradient-to-r from-neutral-900 to-neutral-600 block"
      ...
      {menuItems.map((item) => {
        if (item.type === "folder") {
          return (
            <div key={item.id} className="relative group px-1 py-4 -my-4">
              <button className="flex items-center gap-1 px-3 py-2 text-sm font-medium text-neutral-600 hover:text-neutral-900 transition-colors rounded-full hover:bg-black/5">
                {item.label}
                <ChevronDown
                  size={14}
              ...
              <div className="absolute top-full left-0 pt-3 w-56 opacity-0 translate-y-2 pointer-events-none group-hover:opacity-100 group-hover:translate-y-0 group-hover:pointer-events-auto transition-all duration-200 z-50">
                ...
                <div className="rounded-2xl bg-white/95 backdrop-blur-xl border border-black/10 shadow-2xl overflow-hidden">
                  <div className="p-2 flex flex-col gap-1">
                    {item.children.map((child: any) => (
                      <button
                        key={child.id}
                        onClick={() => handleNavClick(child)}
                        className="text-left px-4 py-2.5 text-sm font-medium text-neutral-600 hover:text-neutral-900 hover:bg-black/5 rounded-xl transition-colors"
                      >
                        {child.label}
                      </button>
                    ))}
        ...
        return (
          <button
            ...
            className="relative px-4 py-2 text-sm font-medium text-neutral-600 hover:text-neutral-900 transition-colors group rounded-full hover:bg-black/5"
          >
            {item.label}
          </button>
        );
  ...
  const SocialLinks = () => (
    <div className="hidden md:flex items-center gap-3 px-3 text-neutral-600 shrink-0">
  ...
  const CTA = () =>
    data.ctaText && (
      <Button
        size="sm"
        asChild
        className="rounded-full bg-neutral-900 text-white hover:bg-neutral-800 px-6 font-semibold h-10 hidden md:flex shrink-0"
      >
        <a
  ...
  const CartButton = () => (
    <button
      ...
      className="relative p-2 text-neutral-600 hover:text-neutral-900 transition-colors shrink-0"
    >
  ...
  return (
    <>
          ...
          {variant === "transparent" && (
            <div className="w-full flex items-center justify-between">
              ...
              <div className="flex items-center gap-2 md:gap-4">
                ...
                <CartButton />
                <div className="pl-4 ml-2 border-l border-black/10 hidden md:block">
                  <CTA />
                </div>
                <button
          ...
          {variant === "centered" && (
            <div className="w-full grid grid-cols-3 items-center">
              <div className="justify-self-start flex items-center">
                <DesktopNav />
                {hasSocial && (
                  <div className="hidden md:block border-l border-black/10 pl-1 ml-4">
                    <SocialLinks />
                  </div>
                )}
                <button
      ...
      <div
        className={cn(
          "fixed inset-0 z-[60] bg-white md:hidden flex flex-col items-center justify-start transition-all duration-500 ease-in-out overflow-y-auto custom-scrollbar",
          isMenuOpen
      ...
      >
        <button
          className="absolute top-6 right-6 p-2 text-neutral-500 hover:text-neutral-900 z-10"
        ...
        <div className="flex flex-col items-center w-full px-8 pt-24 pb-20 gap-8 min-h-full">
          {menuItems.map((item, idx) => {
              ...
              return (
                ...
                >
                  <button
                    ...
                    className={cn(
                      "flex items-center gap-2 text-3xl font-bold text-neutral-800 hover:text-neutral-900 transition-all duration-300 transform",
                      isMenuOpen
                  ...
                  >
                    {item.children.map((child: any) => (
                      <button
                        key={child.id}
                        onClick={() => handleNavClick(child)}
                        className="text-xl font-medium text-neutral-500 hover:text-neutral-900 transition-colors"
                      >
                        {child.label}
                      </button>
                    ))}
            ...
            return (
              <button
                ...
                onClick={() => handleNavClick(item)}
                className={cn(
                  "text-3xl font-bold text-neutral-800 hover:text-neutral-900 transition-all duration-300 transform",
                  isMenuOpen
                    ? "translate-y-0 opacity-100"
                    : "translate-y-8 opacity-0"
                )}
          ...
          {hasSocial && (
            <div
              className={cn(
                "flex items-center justify-center gap-6 mt-8 pt-8 border-t border-black/10 w-full max-w-[200px] transition-all duration-500",
                isMenuOpen
                  ? "translate-y-0 opacity-100"
                  : "translate-y-8 opacity-0"
              )}
          ...
          {data.ctaText && (
            ...
            >
              <Button
                size="lg"
                className="w-full rounded-full bg-neutral-900 text-white hover:bg-neutral-800 text-lg h-14"
                asChild
              >
                <a
                  href={data.ctaLink || "#contact"}
